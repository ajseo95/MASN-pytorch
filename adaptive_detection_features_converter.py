...
def load_image_ids(base_dir):
    ''' Load a list of (path,image_id tuples). Modify this to suit your data locations. '''
    ids = set()
    for gif_dir in os.listdir(base_dir):
        img_dir = os.path.join(base_dir, gif_dir)
        for img_name in os.listdir(img_dir):
            gif_name = gif_dir.split('.')[0]
            image_id = img_name.split('.')[0]
            ids.add(gif_name + '/' + image_id)
    return ids
...
def extract(split, infiles):
    ...
    data_file = {
        'all': './tgif_all_btmup_f.hdf5'
    }
    indices_file = {
        'all': './tgif_all_imgid2idx.pkl'
    }
    ids_file = {
        'all': './tgif_all_ids.pkl'
    }
    path_imgs = {
        'all': '/root/data/TGIFQA/frames',
    }
    known_num_boxes = {
        'all': None
    }
    ...
    if os.path.exists(ids_file[split]):
        ...
        imgids = load_image_ids(path_imgs[split])
    ...
    for infile in infiles:
        ...
        with open(infile, "r+") as tsv_in_file:
            ...
            for item in reader:
                item['num_boxes'] = int(item['num_boxes'])
                item['boxes'] = bytes(item['boxes']).encode('utf-8')
                item['features'] = bytes(item['features']).encode('utf-8')
                image_id = str(item['image_id'])
...
if __name__ == '__main__':
    infile = ['./tgifqa_1.tsv.0', './tgifqa_1.tsv.1']
    extract('all', infile)
