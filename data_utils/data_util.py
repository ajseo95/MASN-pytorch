...
def pad_sequences(
        sequences, pad_token="[PAD]", pad_location="LEFT", max_length=None):
    """
...
def convert_sent_to_index(sentence, word_to_index):
    ...
    """
    return [
        word_to_index[word] if word in list(word_to_index.keys()) else 0
...
def fsr_iter(fsr_data, batch_size, random_seed=42, fill=True):
    ...
    train_iter = batch_iter(
        list(zip(*fsr_data)), batch_size, fill=fill, seed=random_seed)
    return [list(zip(*batch)) for batch in train_iter]
...
def preprocess_sents(descriptions, word_to_index, max_length):
    ...
    descriptions = [
        convert_sent_to_index(sent, word_to_index) for sent in descriptions
    ]
...
def preprocess_roots(roots, word_to_index):
    ...
    roots = [
        word_to_index[root] if root in list(word_to_index.keys()) else 0
        for root in roots
    ]
...
def pad_video(video_feature, dimension):
    ...
    if num_padding == 0:
        ...
        steps = np.linspace(
            0, current_length, num=max_length, endpoint=False, dtype=np.int32)
...
def video_3d_pad(sequence, obj_max_num, max_length):
    ''' Pad sequence with 0.
    '''
...
def question_pad(sequence, max_length):
    ''' Pad sequence with 0.
    '''
    sequence = np.array(sequence)
    sequence_shape = np.array(sequence).shape
...
def dict_keys_bytes2string(dic):
